...
(function () {
    define(['jquery', 'common/js/utils'], function ($, utils) {
        ...
        function componentClick(event) {
            ...
            var dom = comObject[comName].add(comInfo[id]['data']);
            if(!(dom instanceof $ && dom.size() == 1)) {
                throw new Error('只能返回1个jquery节点');
            }
            comInfo[id] = {comName: comName, id: id, data: {}};
            comInfo[id]['basicCom'] = utils.packageBasicCom(dom, id, comName);
        ...
        function basicComClick(event) {
            ...
            editors.children('.basic-edt').hide();
            //调起编辑器
        ...
        function callEditors(_arr, _info) {
            for (var i = 0, l = _arr.length; i < l; i++) {
                ...
                if (!edtInfo[edtName]) {
                    ...
                    var id = utils.uniqueMarker();
                    var dom = instance.instance;
                    if(!(dom instanceof $ && dom.size() == 1)) {
                        throw new Error('只能返回1个jquery节点');
                    }
                    edtInfo[edtName] = {edtName: edtName, id: id, instance: instance};
                    edtInfo[edtName]['basicEdt'] = utils.packageBasicEdt(dom, id, edtName);
